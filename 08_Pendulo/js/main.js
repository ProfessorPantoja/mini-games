...
function showScreen(name) {
  ...
  $("hud").hidden = !(game.state === "playing" || game.state === "flying");
}

function refreshHud() {
  $("hud-level").textContent = String(game.levelIndex + 1);
  $("hud-score").textContent = String(game.score);
  $("hud-tries").textContent = String(game.tries);
...
// Fit canvas CSS size already via CSS; keep internal res fixed
function resize() {
  // logical size fixed 960x640 — CSS scales
}
